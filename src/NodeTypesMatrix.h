...
template<>
class NodeVar<Matrix> : public Node<Matrix>
{
public:
	...
	virtual Matrix evaluate() const {
		throw std::logic_error("cannot evaluate a variable name");
		return Matrix();
	}

	virtual bool evaluate(Matrix &value) const {
		return false;
	}
...
template<>
class NodeOut<Matrix> : public NodeVar<Matrix>
{
public:
	NodeOut (const std::string &varName, Sp<const Node<Matrix>> node)
		: mNode(node), NodeVar<Matrix>(varName) {
		this->init();
	}
